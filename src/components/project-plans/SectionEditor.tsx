...
import { Bold, Italic, List, ListOrdered, Heading3, Heading4, Link2, Table as TableIcon, ChevronDown, Library, BookmarkPlus } from "lucide-react";
...
export default function SectionEditor({ section, onTitleChange, onContentChange }: Props) {
  ...
  // Update editor when section changes externally
  useEffect(() => {
    ...
    <div className="space-y-3">
      ...
      <div className="flex gap-2">
        {libraryItems.length > 0 && (
          <DropdownMenu>
            <DropdownMenuTrigger asChild>
              <Button variant="outline" size="sm" className="text-xs gap-1 bg-erp-bg3 border-erp-border0 text-erp-text1">
        ...
        <Button variant="outline" size="sm" className="text-xs gap-1 bg-erp-bg3 border-erp-border0 text-erp-text1" onClick={saveToLibrary}>
          <BookmarkPlus className="w-3.5 h-3.5" /> Opslaan als bibliotheek-item
        </Button>
      </div>
    </div>
  );
